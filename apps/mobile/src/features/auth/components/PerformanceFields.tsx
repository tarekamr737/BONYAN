...
export function performanceError(value: CoachingPreferences): boolean {
  return [value.running_target_minutes, value.pushups_target, value.pullups_target].some(v => v != null && v <= 0)
    || (value.target_weight_kg != null && value.target_weight_kg < 30)
    || [value.active_days_per_week, value.pushups, value.pushups_target, value.pullups, value.pullups_target].some(v => v != null && !Number.isInteger(v));
}

export function PerformanceFields({goal, value, onChange, arabic = false}: {goal: TrainingGoal; value: CoachingPreferences; onChange: (value: CoachingPreferences) => void; arabic?: boolean}) {
