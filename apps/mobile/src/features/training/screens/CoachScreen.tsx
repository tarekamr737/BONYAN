...
export function CoachScreen() {
  ...
  return <CoachingPage arabic={arabic} footer={<View style={{gap: 8}}><AppTextField label={arabic ? "رسالتك لبنيان" : "Your message to Bonyan"} multiline value={message} editable={!send.isPending} onChangeText={setMessage} maxLength={1000} placeholder={arabic ? "اسأل عن تدريبك…" : "Ask about your training?"} /><AppButton label={arabic ? "إرسال" : "Send"} loading={send.isPending} disabled={!message.trim()} onPress={() => { if (busy.current) return; busy.current = true; void send.mutateAsync(message.trim()).catch(() => {}).finally(() => {busy.current = false;}); }} /></View>}>
    ...
    {history.isPending ? <Text accessibilityLiveRegion="polite" style={ui.small}>{arabic ? "?????? ?????????" : "Loading conversation?"}</Text> : null}
    {history.isError ? <GlassCard><Text accessibilityRole="alert" style={ui.error}>{arabic ? "????? ????? ???????? ????????." : "Couldn?t load your saved conversation."}</Text><AppButton label={arabic ? "????? ????????" : "Retry"} variant="secondary" onPress={() => void history.refetch()} /></GlassCard> : null}
    {history.data?.length === 0 ? <GlassCard><Text style={ui.heading}>{arabic ? "???? ?????" : "Where shall we start?"}</Text>{(arabic ? ["???? ?? ??? ??????", "?????? ????? ???? ??????"] : ["Explain my training plan", "Help me find an exercise alternative"]).map(prompt => <AppButton key={prompt} label={prompt} variant="secondary" onPress={() => setMessage(prompt)} />)}</GlassCard> : null}
    {history.data?.map(entry => <GlassCard key={entry.id}><Text style={ui.small}>{entry.role === "coach" ? arabic ? "?????" : "Bonyan" : arabic ? "???" : "You"}</Text><Text selectable style={ui.text}>{entry.role === "coach" ? readableCoachText(entry.content) : entry.content}</Text></GlassCard>)}
