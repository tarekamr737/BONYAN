...
export type JourneyStep = "welcome" | "name" | "goal" | "military" | "focus" | "body" | "experience" | "performance" | "schedule" | "summary" | "gym-goal" | "test-date" | "running" | "pushups" | "situps" | "pullups" | "limitations" | "equipment";
export function journeySteps(goal: TrainingGoal): JourneyStep[] {
 return ["welcome", "name", "goal", ...(goal === "military_preparation" ? ["military", "test-date", "running", "pushups", "situps", "pullups", "limitations"] as const : ["gym-goal", "focus"] as const), "experience", "schedule", "equipment", "body", "performance", "summary"];
}
export function performanceFields(goal: TrainingGoal) {
 return {running: ["general_fitness", "fat_loss", "military_preparation"].includes(goal), pushups: ["strength", "hypertrophy", "military_preparation"].includes(goal), situps: goal === "military_preparation", pullups: goal === "military_preparation", weightTarget: goal === "fat_loss"};
}
export function cleanCoaching(goal: TrainingGoal, value: CoachingPreferences): CoachingPreferences {
 return {...value, ...(goal !== "military_preparation" ? {military_subtype: null, target_date: null, situps: null, situps_target: null, pullups: null, pullups_target: null, limitations: null} : {}), ...(goal !== "fat_loss" ? {target_weight_kg: null} : {})};
}
