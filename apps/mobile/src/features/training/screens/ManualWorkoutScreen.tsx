...
export function ManualWorkoutScreen() {
  ...
  const create = useMutation({
    mutationFn: () => createManualWorkoutPlan({
      ...
      exercises: selected.map(item => ({ exercise_id: item.id, sets: 3, reps_min: 8, reps_max: 12, rest_seconds: 90 })),
      activate: true,
    }),
    onSuccess: plan => router.replace({ pathname: "/training/day", params: { dayKey: plan.days[0]?.key } }),
  });
  ...
  return <CoachingPage arabic={arabic} footer={<AppButton disabled={!selected.length || create.isPending} label={create.isPending ? arabic ? "جاري تجهيز التمرين…" : "Preparing workout…" : arabic ? `ابدأ التمرين (${selected.length})` : `Start workout (${selected.length})`} onPress={() => create.mutate()} />}>
    ...
    <View style={styles.heading}><Text accessibilityRole="header" style={[ui.title, arabic && ui.rtl]}>{arabic ? "ابنِ تمرينك" : "Build your workout"}</Text><Text style={[ui.text, arabic && ui.rtl]}>{arabic ? "ابحث واختر التمارين. سنجهز 3 مجموعات لكل تمرين ويمكنك تسجيل الوزن والتكرارات أثناء الجلسة." : "Search and choose exercises. We will prepare three sets each; log weight and reps during the session."}</Text></View>
    ...
    {selected.length ? <View style={[styles.selection, arabic && styles.reverse]}><Text style={styles.selectionText}>{arabic ? `${selected.length} تمارين مختارة` : `${selected.length} selected`}</Text><Pressable onPress={() => setSelected([])}><Text style={styles.clear}>{arabic ? "مسح" : "Clear"}</Text></Pressable></View> : null}
