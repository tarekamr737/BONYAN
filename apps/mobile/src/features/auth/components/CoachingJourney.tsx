...
export function CoachingJourney({profile, editing = false, onDone}: {profile: UserProfile; editing?: boolean; onDone?: () => void}) {
  ...
  if (complete) return <CoachingPage arabic={arabic}><Text style={ui.title}>{arabic ? "خطوتك القادمة جاهزة" : "Your next chapter starts here."}</Text><GlassCard><Text style={ui.heading}>{draft.displayName}</Text><Text style={ui.text}>{goalLabel(draft.trainingGoal, arabic)}</Text><Text style={ui.text}>{draft.availableTrainingDays} {coachingCopy("days / week", arabic)} · {coachingCopy(draft.experienceLevel, arabic)}</Text><Text style={ui.small}>{overview.data?.latest ? `Body data: ${overview.data.latest.snapshot.source}` : "Add body data whenever you’re ready."}</Text></GlassCard>{overview.data ? <ScoreCard score={overview.data.score} arabic={arabic} /> : null}<AppButton label={arabic ? "ابدأ التدريب" : "Explore my training"} onPress={() => {client.setQueryData(["profile", "me"], save.data); onDone?.(); router.replace("/training");}} /></CoachingPage>;
