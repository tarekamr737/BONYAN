...
export function CoachingJourney({profile, editing = false, onDone}: {profile: UserProfile; editing?: boolean; onDone?: () => void}) {
  ...
  return <CoachingPage arabic={arabic} step={step} footer={<>{error ? <Text accessibilityRole="alert" style={ui.error}>{error}</Text> : null}
    ...
    {step === "limitations" ? <><Text style={[ui.text, arabic && ui.rtl]}>{arabic ? "اختياري. اذكر أي إصابة أو حركة تحتاج تعديلًا حتى تراعيها خطتك." : "Optional. Add any injury or movement that needs an adjustment so your plan can account for it."}</Text><AppTextField label={arabic ? "الإصابات أو القيود · اختياري" : "Injuries or limitations · optional"} value={coaching.limitations ?? ""} maxLength={500} onChangeText={value => setCoaching({...coaching, limitations: value.trimStart() || null})} /></> : null}
