...
class AvatarService:
    ...
    async def create(self, owner_id: str, request: CreateAvatarRequest) -> AvatarView:
        source = decode_source_image(request.source_image_base64, request.source_media_type)
        source_key = await self._storage.put_private(source.content, source.media_type)
        ...
        try:
            ...
            await self._storage.delete_private(source_key)
            raise

        await self._generate(avatar, source.content)
    ...
    async def regenerate(self, owner_id: str, avatar_id: UUID) -> AvatarView:
        ...
        source_content = await self._storage.get_private(avatar.source_object_key)
        avatar.is_public = False
        await self._generate(avatar, source_content)
        return await self._to_view(avatar)
    ...
    async def delete(self, owner_id: str, avatar_id: UUID) -> None:
        avatar = await self._require_owned(owner_id, avatar_id)
        keys = [avatar.source_object_key]
        if avatar.generated_object_key:
            keys.append(avatar.generated_object_key)
        try:
    ...
    async def _generate(self, avatar: AvatarRecord, source_content: bytes) -> None:
            ...
            result = await asyncio.wait_for(
                self._provider.generate(
                    AvatarGenerationRequest(
                        ...
                        source_media_type=avatar.source_media_type,
                        style=avatar.style,
                    )
                ),
